...
export function StocksPage() {
  ...
  const handleSearch = async (e) => {
    ...
    setError('');
    try {
      const response = await stockService.getStockPrice(query);
      setResult(response.data);
    } catch (err) {
      setError(err.response?.data?.detail || 'Failed to fetch stock price');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-app page-fade min-h-screen pb-24 md:pb-10">
      <div className="container mx-auto max-w-6xl px-4 py-10">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">Stock Prices</h1>
          <p className="text-slate-300">Get real-time stock price information</p>
        </div>
              ...
              <div className="glass-panel rounded-3xl p-8">
                ...
                <div className="space-y-4">
                  {result.steps.map((step, index) => (
                    <div key={index} className="rounded-2xl bg-black/35 p-4 border-l-4 border-lime-400">
                      <div className="text-sm font-semibold text-lime-300 mb-2">Step {index + 1}</div>
